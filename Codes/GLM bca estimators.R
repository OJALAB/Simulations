...
for(i in 1:n_sim){
  ...
  naive <- glm(y ~ as.factor(z_star) + x, family = "binomial")
...
colMeans(biases_precise)
colMeans(biases_naive)
colMeans(biases_bca)
...
save(results, biases_precise, biases_naive, biases_bca, file = "Results/GLM bca estimators.RData")
