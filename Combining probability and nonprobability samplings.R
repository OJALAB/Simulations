...
for(i in 1:n_sim){
  ...
  #gold standard estimator - uses precise values from probability sample, impossible in practice
  ...
  #mass imputation estimator - uses precise values from non-probability sample to estimate values of probability sample, impossible in practice
  ...
  bcm_coef = bcm$coef
  y_pred_bcm = bcm_coef[3]+bcm_coef[2]*prob_sample[['x']]+bcm_coef[1]*prob_sample[['z']]
  bias_bcm = mean(y_pred_bcm) - y_mean
  biases[i, 6] = bias_bcm
  ...
  y_pred_bcm_upgr = bcm_upgr[2] + bcm_upgr[3]*prob_sample[['x']]+bcm_upgr[1]*prob_sample[['z']]
  bias_bcm_upgr = mean(y_pred_bcm_upgr) - y_mean
  biases[i, 7] = bias_bcm_upgr
  
  #joint estimator - uses joint estimation to correct measurement error
  joint = MLBC::one_step(y ~ z_star + x, data = nonprob_sample)
  joint_coef = joint$coef
  y_pred_joint = joint_coef[3] + joint_coef[2]*prob_sample[['x']]+joint_coef[1]*prob_sample[['z']]
